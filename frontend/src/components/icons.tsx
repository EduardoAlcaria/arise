...
function LogoImg({ domain, size, fallback }: { domain: string; size: number; fallback: React.ReactNode }) {
  ...
  return (
    <img
      ...
      style={{ borderRadius: Math.round(size * 0.22), objectFit: 'contain', flexShrink: 0, display: 'block' }}
      alt={domain}
    />
  )
}
...
export function OsIcon({ name, size = 38 }: { name: string; size?: number }) {
  ...
  if (!cfg.domain) return fallback
  return (
    <div style={{ width: size, height: size, borderRadius: radius, flexShrink: 0, overflow: 'hidden', background: cfg.bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <LogoImg domain={cfg.domain} size={size} fallback={fallback} />
    </div>
  )
}
...
export function GitHubIcon({ size = 22, className = '' }: { size?: number; className?: string }) {
  ...
  return <LogoImg domain="github.com" size={size} fallback={svgFallback} />
}
